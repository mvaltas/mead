...
temp_lost_to_env = ms.Flow("Lost to env", formula=lambda: 0.01 * (temp.value - env_temp))

# heater power when activated
temp_heater = ms.Constant(
    "Heater power", 
...
smoothed_heater = ms.SmoothedAuxiliary("Heater ramp", heater_signal_delayed, tau_steps=10)
